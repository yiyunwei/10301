...
def trim(dictionary, data):
    trimmed = dict()
    (label, review) = data
    array = review.split(" ")
    for word in array:
        if(word in dictionary):
            trimmed[word] = dictionary[word]

    return trimmed

def featVector(dictionary, data):
    count = 0
    vector = [0] * VECTOR_LEN
    (label, review) = data
    array = review.split(" ")
    ...
    for i in range(len(vector)):
        ...
        vector[i] = round(float(vector[i]), 6)
    return vector

def main():
    glove_map = load_feature_dictionary(args.feature_dictionary_in)
    trainIn = load_tsv_dataset(args.train_input)
    validIn = load_tsv_dataset(args.validation_input)
    testIn = load_tsv_dataset(args.test_input)
    f1 = open(args.train_out, "w+")
    f2 = open(args.validation_out, "w+")
    f3 = open(args.test_out, "w+")

    for i in range(len(trainIn)):
        smalldict = trim(glove_map, trainIn[i])
        (label, review) = trainIn[i]
        vect = featVector(smalldict, trainIn[i])
        f1.write(str(label))
        for v in vect:
            f1.write("\t" + str(v))
        f1.write("\n")
    ...
    for i in range(len(validIn)):
        smalldict = trim(glove_map, validIn[i])
        (label, review) = validIn[i]
        vect = featVector(smalldict, validIn[i])
        f2.write(str(label))
        for v in vect:
            f2.write("\t" + str(v))
        f2.write("\n")
    ...
    for i in range(len(testIn)):
        smalldict = trim(glove_map, testIn[i])
        (label, review) = testIn[i]
        vect = featVector(smalldict, testIn[i])
        f3.write(str(label))
        for v in vect:
            f3.write("\t" + str(v))
        f3.write("\n")
